...
from transformers.modeling_bert import BertForSequenceDisfluency_real
...
class DisfluencyProcessor(DataProcessor):
    ...
    def get_unused_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "unused.tsv")),
            "unused")
...
def unused_tagging(eval_examples, predict_result_tagging, predict_result_logits, gold_result_tagging, input_mask_tagging, output_name):
    ...
    for i in range(0, len(predict_result_tagging)):
        ...
        predict_logits = predict_result_logits[i]
        ...
        for j in range(0, len(predict_results)):
            ...
            for k in range(0, len(gold_result_tmp)):
                if gold_result_tmp[k] != -1:
                    ...
                    predict_logit.append(str(round(float(predict_logits_tmp[k]), 5)))
            if len(text_a) != len(predict_result):
                logger.info(len(text_a))
                logger.info(text_a)
                logger.info(len(predict_result))
                logger.info(predict_result)
                text_a = text_a[:len(predict_result)]
            ...
            for l in range(0, len(text_a)):
                ...
                gold_tokens.append(gold_label)
            output_file.write(str(id)+"\t"+" ".join(text_a)+"\tNONE\tNONE\t"+" ".join(gold_tokens) + "\t" + " ".join(output_tokens)+ "\t" + " ".join(predict_logit) + "\n")
            id += 1
    output_file.close()
...
def main():
    ...
    parser.add_argument("--do_unlabel",
                        action='store_true',
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--do_unused_predict",
                        action='store_true',
                        help="Whether to run eval on the dev set.")
    ...
    # x1 = torch.FloatTensor(2400, 1000, 1000).cuda(0)
    # del x1
    ...
    if args.use_new_model:
        ...
        model = BertForSequenceDisfluency_real.from_pretrained(
            pretrained,
            config=config,
            cache_dir=args.cache_dir if args.cache_dir else None,
            state_dict=state,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                logger.info("***** Running evaluation on test of epoch %d *****", epoch_size)
                logger.info("  Num examples = %d", len(eval_examples))
                logger.info("  Batch size = %d", args.eval_batch_size)
                all_input_ids = torch.tensor([f.input_ids for f in eval_features], dtype=torch.long)
                all_input_mask = torch.tensor([f.input_mask for f in eval_features], dtype=torch.long)
                ...
                if args.do_tagging:
                    ...
                    with open(output_eval_file, "w") as writer:
                        logger.info("***** Test Eval results %d*****", epoch_size)
                        for key in sorted(result.keys()):
                            logger.info("  %s = %s", key, str(result[key]))
                            writer.write("%s = %s\n" % (key, str(result[key])))
    ...
    # &&&& 给数据打标签
    if args.do_unused_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        unused_examples = processor.get_unused_examples(args.data_dir)
        random.shuffle(unused_examples)
        ...
        unused_features = convert_examples_to_features(
            unused_examples, label_list, label_disf_list, args.max_seq_length, tokenizer, args.sel_prob, "unlabel")
        logger.info("***** Running evaluation on dev of epoch *****")
        logger.info("  Num examples = %d", len(unused_examples))
        logger.info("  Batch size = %d", args.train_batch_size)
        all_input_ids = torch.tensor([f.input_ids for f in unused_features], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in unused_features], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in unused_features], dtype=torch.long)
        all_label_disf_ids = torch.tensor([f.label_disf_id for f in unused_features],
                                          dtype=torch.long)
        ...
        for input_ids, input_mask, segment_ids, label_disf_ids in tqdm(unlabel_dataloader,
            ...
            logits_tagging = logits_tagging.detach().cpu()
            input_mask = input_mask.to('cpu').numpy()
            label_disf_ids = label_disf_ids.to('cpu').numpy()

            gold_result_tagging.append(label_disf_ids.tolist())
            predict_result_tagging.append(np.argmax(logits_tagging, axis=-1).tolist())
            predict_softmax = F.softmax(logits_tagging, dim=-1).tolist()
            # logger.info("predict_softmax:{}".format(predict_softmax))
            predict_result_logits.append([[max(x[0], x[1]) for x in batch] for batch in predict_softmax])
            # logger.info("predict_result_logits:{}".format(predict_result_logits))
            # exit(0)
            input_mask_tagging.append(input_mask.tolist())

        # logger.info(predict_result_logits)
        unused_tagging(unused_examples, predict_result_tagging, predict_result_logits, gold_result_tagging,
                        input_mask_tagging, os.path.join(args.output_dir, "unused_results.txt"))
    # # &&&& 给数据打标签
